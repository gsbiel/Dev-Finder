...
class UserScreen extends Component {
    ...
    async componentDidMount(){
        ...
        try{
            const resp = await axios_git.get('/user',{
                headers:{
                    Authorization: 'Bearer ' + this.props.token
                }
            })
            const user = {
                name: resp.data.name !== "" && resp.data.name !== null ? resp.data.name : '---',
                username: resp.data.login !== "" && resp.data.login !== null ? resp.data.login : '---',
                followers: resp.data.followers !== "" && resp.data.followers !== null ? resp.data.followers : '---',
                site: resp.data.html_url,
                email: resp.data.email !== "" && resp.data.email !== null ? resp.data.email : '---',
            ...
            const resp2 = await axios_git.get('/users/'+user.username+'/repos',{
                headers:{
                    Authorization:'Bearer ' + this.props.token
                }
            });
        ...
        Geolocation.getCurrentPosition(
            (position) => {
                ...
                axios.get('https://maps.googleapis.com/maps/api/geocode/json?latlng='+latitude+','+longitude+'&key='+API_KEY)
                    .then(resp => {
                        const cidade = resp.data.results[0].address_components[3].short_name;
                        const estado = resp.data.results[0].address_components[4].short_name;
                        const position = {
                            city: cidade,
                            state: estado
                        };
                    ...
                    <View style={styles.profileLayout}>
                        <View style={styles.devNameView}>
                            <Text style={{
                                ...
                                fontSize:24,
                                fontWeight:'bold'
                            }}>{this.props.dev.name}</Text>
                        </View>
                        <View style={styles.imageView}>
                            <Image style={{width:'100%', height:'100%', borderRadius:70}} source={{uri:this.props.dev.image}}/>
                        </View>

                        <View style={styles.sectionA}>
                            <View style={styles.cardA1}>
                                {this.props.local.city&&this.props.local.state&&this.state.isGPSAllowed ?  
                                    <Text style={styles.locationLabel}>
                                        {this.props.local.city}-{this.props.local.state}
                                    </Text> :
                                    <TouchableOpacity onPress={() => this.getLocation()}>
                                        <Text style={styles.getLocationBtn}>
                                            Obter Localização
                                        </Text>
                                    </TouchableOpacity>
                                }   
                            ...
                            <View style={styles.cardA2}>
                                <View style={{
                                    ...
                                    <Table rowNumber={3} tableWidth={160}>
                                        <TableRow label="Username" value={this.props.dev.username} />
                                        <TableRow label="Seguidores" value={this.props.dev.followers} />
                                        <TableRow label="Repositórios" value={this.props.dev.repositories} />
                                    </Table>
                                    <Button disabled={!this.state.isGPSAllowed} title="Busque um Dev!" color={colors.themeColor}/>
                                </View>
                            </View>
                        </View>
                        ...
                        <View style={styles.sectionB}>
                            ...
                            <SlidingTab position={{top:20, left:'5%'}} labelTab1='Repositórios' labelTab2='Favoritos' scrollViewWidth = {this.scrollViewWidth} scroll={this.scrollHandler} />
                            ...
                            <View style={styles.cardB2}>
                                <View style={{
                                    ...
                                    <ScrollView 
                                        ...
                                        <View style={{backgroundColor:colors.themeColor,height:'100%',width:this.scrollViewWidth, borderRadius:20}}>
